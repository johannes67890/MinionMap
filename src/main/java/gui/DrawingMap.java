...
public class DrawingMap {
    ...
    public DrawingMap(MainView mainView){
        this.mainView = mainView;
        nodes = XMLReader.getNodes().valueCollection().stream().toList();
    ...
    private void drawWays(MinPQ<TagWay> ways){
        ...
        while (!ways.isEmpty()) {
            ...
            nodesRef = tagWay.getRefs().toArray(new TagNode[0]);
    ...
    public void handleRelations(){
        for (TagRelation relation : relations){
            ...
            handleWays(relation.getWays().valueCollection().stream().toList());
